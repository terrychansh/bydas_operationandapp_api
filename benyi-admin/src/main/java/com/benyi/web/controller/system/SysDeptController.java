...
import com.benyi.system.service.ISysRoleService;
...
public class SysDeptController extends BaseController
{
    ...
    public AjaxResult list(SysDept dept)
    {
        ...
        for(SysDept de:depts){
            ...
            List<String> returnPlantList= new ArrayList<String>();
            SysUser queryUser = new SysUser();
            queryUser.setDeptId(de.getDeptId());
            List<SysUser> userList=userService.selectUserListForDeptIndex(queryUser);
            for(SysUser user:userList){
                String roleName=userService.selectUserRoleGroup(user.getUserName());
                String add="";
                ...
                //System.out.println();
                userNameList.add(user.getUserName()+"|"+add);
            }
            de.setUserNameList(userNameList);
            CidPowerstationinfo queryPower = new CidPowerstationinfo();
            queryPower.setDelFlag("0");
            queryPower.setEnergyDeptId(de.getDeptId());
            List<CidPowerstationinfo> plantList=powerstationinfoService.selectCidPowerstationinfoList(queryPower);
            for(CidPowerstationinfo plant:plantList){
                String add = plant.getEnergyName()+"|"+plant.getId();
                returnPlantList.add(add);
            }
            de.setPlantList(returnPlantList);
        }
